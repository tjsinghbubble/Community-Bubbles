...
- **Web Admin Pages**: Super admins and bubble admins see an Administration section on the web Profile page (with "Super Admin"/"Admin" badge). Two sub-pages: `/admin/pending` (tabbed view: pending bubbles, events, waitlist requests, reports — with approve/reject actions) and `/admin/rules` (tabbed: app-wide rules + per-category rules, full CRUD). Routes registered in `client/src/App.tsx`.
