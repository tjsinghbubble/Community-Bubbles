...
HELP = """\
bubble_server_windows — set up Terminal windows for local development

Commands:
  create           Create all windows (default when no command given)
  kill             Close all windows created by this script
  resize           Re-apply computed bounds to existing windows
  show_boundaries  Print screen size and computed window bounds
  help             Show this message

Window names  (pass one name to act on only that window):
  API Server
  Metro Bundler
  iOS App
  Android App
...
# ---- Main ----
def main():
    global screen_left, screen_top, screen_width, screen_height
    args = parse_args()
